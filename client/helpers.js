...
Template.mturkSubmit.rendered = function() {
...
Template.tsTimeOptions.helpers({
  ...
  valueFormatted() {
    return this.zone(300).format("HH ZZ");
  },
  // Display values in user's timezone
  displayFormatted() {
    return this.local().format("hA [UTC]Z");
  }
});
...
TurkServer.submitExitSurvey = (results, panel) =>
  Meteor.call("ts-submit-exitdata", results, panel, function(err, res) {
    if (err) {
      bootbox.alert(err);
    }
